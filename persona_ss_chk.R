                         ...
                         tabItem(
                           ...
                           fluidRow(
                             ...
                             box(title = "Model Framework", status = "primary", width = 5, solidHeader = TRUE, collapsible = TRUE,
                                 ...
                                 column(12, selectInput("type","Algorithm",choices=list("rpart","ctree"))),
...
server <- function(session, input, output) {
  ...
  observeEvent(paste0(input$hyp_bool, input$type), {
  ...
  observeEvent(input$action3, {
    ...
    if(input$modltyp == 2) {
      ...
      if (length(unique(p_mod$model_df[,input$colname_t]))==3){
      ...
      } else if  (input$type == "ctree"){
        
        
        
        gs <- list(minsplit = minsplit_i,
                   maxdepth = maxdepth_i) %>% 
          cross_df() # Convert to data frame grid
        
        
        mod <- function(...) {
          ctree(Event_sb ~ ., data = train, control = ctree_control(...))
        }
        ...
        compute_accuracy <- function(fit, test_features, test_labels) {
          predicted <- predict(fit, test_features, type = "response")
          mean(predicted == test_labels)
        }
        
        train_features <- train %>% select(-Event_sb)
        train_labels   <- train$Event_sb
        
        


        gs <- gs %>%
          mutate(train_accuracy =  purrr::map_dbl(fit,compute_accuracy,train_features,train_labels))

        gs <- gs %>% arrange(desc(train_accuracy), desc(minsplit), maxdepth)
        gs1<-gs[1,]

        xplr=ctree_control(minsplit = gs1$minsplit,maxdepth = gs1$maxdepth )

        optimal_tree<-ctree(formula=Event_sb ~ ., data=train, control=xplr)
        p_mod$optimal_tree2 <- optimal_tree



        # tot_obs <- sum(nrow(train))
        tab <- tapply(train$Event_sb, predict(optimal_tree, type = "node"),
        ...
        q<-list(partykit:::.list.rules.party(optimal_tree))
        q1<-list(q[[1]])
        mylist = do.call("cbind",q1)
        frm1<- as.data.frame(mylist)
        names(frm1)[1] <- "rule"
        frm1$node<-row.names(frm1)


        if (length(unique(p_mod$model_df[,input$colname_t]))==3){
          # Predicting the classifier on train

          y_pred_train = predict(object=optimal_tree, newdata = train, type = 'response')
          train_t<-table(train$Event_sb,y_pred_train)
          result_train<-confusionMatrix(train_t)
          Confusion_metrc_train<-data.frame(result_train$byClass)
          Confusion_metrc_trainn <- as.data.frame(t(Confusion_metrc_train))
          ...
          y_pred = predict(object=optimal_tree, newdata = test, type = 'response')
          test_t <- table(test$Event_sb,y_pred)
          result_test<-confusionMatrix(test_t)
          Confusion_metrc_test<-data.frame(result_test$byClass)
          Confusion_metrc_testt <- as.data.frame(t(Confusion_metrc_test))
          ...
          p_mod$model_conf_mat2 <- Confusion_matrix

          #terminal node creation
          ds.size<-sum(nrow(train))
          frm$coverage<-frm$n
          # frm$coverage_per<- cat(sprintf("%.0f%%",round(100*frm[1:nrow(frm),]$n/ds.size)))
          frm$coverage_per<- round(100*(frm$n/ sum(nrow(train))),1)
          # frm$hit_rate<-frm$yval
          frm$hit_rate<-frm$`Y`
          frm$non_eventrt<-frm$`N`
          frm$indetrt<-frm$`I`
          frm$Event_vol<-round((frm$n * frm$`Y`)/100)
          frm$NonEvent_vol<-round((frm$n * frm$`Y`)/100)
          frm$Indeterminate_vol<-round((frm$n * frm$`I`)/100)


          final_table<-cbind(frm1, frm)
          final_table$rule <- gsub("\"|\\)|\\(","", final_table$rule)
          final_table$rule <- gsub("%in%,","", final_table$rule)
          # final_table2 = data.frame(lapply(final_table, as.character), stringsAsFactors=FALSE)

          p_mod$model_final_table2 <- final_table
          ...
          p_mod$Terminal_node2 <- Terminal_node_dt
  ...
  output$model_plot <- renderPlot({
    ...
    if (input$type=="rpart"){
      rpart.plot(p_mod$optimal_tree, type = 4, extra = 109, clip.right.labs = F,
        ...
        ctree<-plot(p_mod$optimal_tree2)
        }
  })
  ...
  output$conf_mat <- DT::renderDataTable({
    ...
    datatable(p_mod$model_conf_mat, rownames= FALSE)
      ...
      datatable(p_mod$model_conf_mat2, rownames= FALSE)
    }
  ...
  output$model_rules <- DT::renderDataTable({
    if (input$type=="rpart"){
      ...
      temp <- p_mod$Terminal_node2[, c("rule", "coverage_per", "hit_rate", "event_perc","cumm_event_perc","KS","gain")]
    }
  ...
  observeEvent(input$drp_var_sb, {
    ...
    if(nrow(df) > 10000) {
    p_mod$clus_df <- dplyr::sample_n(df, 10000)
    } else {
      p_mod$clus_df <- df
    }
  ...
  output$tsne_sb <- renderPlotly({
    ...
    pal_sb1 <- brewer.pal(n = 8, name = "Dark2")
    pal_sb2 <- brewer.pal(n = 9, name = "Set1")
